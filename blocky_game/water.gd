...
const ENABLE_WATER_SPREAD = false
...
func _process_cell(pos: Vector3):
	...
	# In build mode, keep water static: no auto-shape conversion, no spread.
	# This prevents placed water from shrinking to `water_top` and flooding worlds.
	if not ENABLE_WATER_SPREAD:
		return
